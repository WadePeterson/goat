import { Sprites, SoundFX } from './constants';
  ...
  update() {
    ...
    switch (this.state) {
      ...
      case JumpState.NOT_JUMPING:
        ...
        if (this.keys.isDown(Actions.Jump)) {
      ...
      case JumpState.JUMPING:
        ...
        if (this.player.body.touching.down) {
          this.state = this.keys.isDown(Actions.Jump) ? JumpState.WAITING_TO_RELEASE_JUMP : JumpState.NOT_JUMPING;
        }
        break;
...
export class Player {
  ...
  constructor(game: Phaser.Game, keys: KeyMap) {
    this.game = game;
    this.keys = keys;
    this.sprite = game.add.sprite(game.world.top, game.world.left, Sprites.MONKEY);
  ...
  update() {
    const rightDown = this.keys.isDown(Actions.MoveRight);
    const leftDown = this.keys.isDown(Actions.MoveLeft);
